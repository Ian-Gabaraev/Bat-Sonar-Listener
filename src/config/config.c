...
int load_config(const char *filename, AppConfig *config) {
    ...
    const char *aws_endpoint = cJSON_GetObjectItem(json, "aws_endpoint")->valuestring;
    ...
    config->frames_per_buffer = cJSON_GetObjectItem(json, "frames_per_buffer")->valueint;

    strncpy(config->device_name, device_name, sizeof(config->device_name));
    strncpy(config->mqtt_topic, mqtt_topic, sizeof(config->mqtt_topic));
    strncpy(config->aws_endpoint, aws_endpoint, sizeof(config->aws_endpoint));
