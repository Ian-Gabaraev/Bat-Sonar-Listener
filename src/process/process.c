...
float get_dominant_freq(const ReaderContext *rc, const size_t num_of_elements) {
  ...
  const float dominant_freq =
      (float)max_index * (float)rc->sampling_rate_hz / (float)num_of_elements;
...
void process(const ReaderContext *rc, struct timespec *start,
             struct timespec *end) {
  ...
  const int amp_max =
      get_amp_max(rc->buffer->storage, SYNCHRONOUS_SINGULAR_BUFFER_SIZE);
  const float dom_freq =
      get_dominant_freq(rc, SYNCHRONOUS_SINGULAR_BUFFER_SIZE);
